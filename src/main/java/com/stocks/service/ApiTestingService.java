...
public class ApiTestingService {
    ...
    public List<StockResponse> callApi() {
        ...
        if (stockList == null || stockList.isEmpty()) return Collections.emptyList();

        try {
            List<Future<StockResponse>> futures = stockList.stream()
                    .map(stock -> executorService.submit(() -> processStock(stock)))
    ...
    private StockResponse processApiResponse(ApiResponse apiResponse, String stock) {
        ...
        return chunks.stream()
                ...
                .limit(chunks.size() - 3)
                .parallel()
                .map(chunk -> evaluateChunk(chunk, previousData, firstCandleHigh, firstCandleLow, stock))
                .filter(Objects::nonNull)
                .findFirst()
                .orElse(null);
    }
...
//            return new StockResponse(stock, "P", recentData.getTime(), oiInterpretation, firstCandleLow, recentData.getClose(), true);
    ...
    private StockResponse getFutureResult(Future<StockResponse> future) {
        try {
            return future.get();
        } catch (Exception e) {
