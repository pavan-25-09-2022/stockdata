...
public class RangeBreakoutStrategy {
	...
	public void breakOutStocks(Properties prop) {
		...
		for (String stock : stockList) {
			...
			ystCandles = ystCandles.subList(1, ystCandles.size());
		...
		if (!"C".equals(res.getStockType())) {
			...
			if (topWick < bodySize && priceNow > firstCandle.getHigh() && isPositive(candles) && lastCandle.getEndTime().isBefore(LocalTime.of(11, 00)) && lastCandle.getOpen() < lastCandle.getClose() && "LBU".equals(lastCandle.getOiInt())) {
				...
				if (earlierOi.isEmpty()) {
					...
					log.info("Bullish Breakout Detected! {} - RSI {} OI {} OI% {} with volume {} is higher {} Open {} close {} Stop-loss: {} at {}", stock, String.format("%.2f", value), lastCandle.getOiInt(), lastCandle.getCurOiPer(), lastCandle.getVolume(), lastCandle.isHighVolume(), lastCandle.getOpen(), lastCandle.getClose(), rangeLow, lastCandle.getEndTime());
				}
